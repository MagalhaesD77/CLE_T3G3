...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < nIter; i++)
    {
        ...
        printf("Current iteration: %d / Length sub-sequence: %d / Grid dimension: %d\n", i, lenSubSeqeunce, (int)pow(2, (nIter - i) - 1));
...
__global__ void bitonicSortOnGPU(int *numArray, int lenSubSequence, int iter)
{
    int x, y, idx;
    ...
    if (lenSubSequence == N){   // bitonic sort
        ...
        for (int k = 2; k <= lenSubSequence; k *= 2){
